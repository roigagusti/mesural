...
function validate_form_custom(formdata, language) {
	$(formdata).attr('action', '/system/form.php?lang=' + language);
	return check(formdata);
}

function check_custom(p_form, lang)
{
    ...
    if (completed) {
		...
		var btn = $(p_form).find('input[type=submit]');
		btn[0].disabled = true;
		if(lang === 'fr') {
			btn[0].value = 'Envoi en cours, veuillez patienter...';
		} else {
			btn[0].value = 'Sending, please wait...';
		}
